...
#This code is a modified version of 12.py, so it may not be perfect in terms of naming and flow, but it is much faster than my original version of 06.py
#There is some error in calculation somewhere that makes this code slightly wrong with its end results (approx. ~$500 off), same for 09.py since it is just a modified version.
#As much as I have tried I cannot find the issue though.
...
for i in range(12):
    for j in all_sales[i].items():
        key = j[1]["ProductId"]
        key2 = j[1]["CustomerId"]
        sales[key].append(tuple((j[1]["Quantity"], rates[key2], int(j[1]["Date"][5:7])-1)))
...
for product in sales.items():
    ...
    for i in product[1]:
        ...
        tax = float(i[1])
        month = i[2]
        annRevs[key].append( tuple((quantity * price * tax, i[2])))

#Make list to hold revenues for each month, corresponding to index (0 = january, 1 = february...)
monthlyRevs = []
for i in range(12):
    monthlyRevs.append(0)

#Iterate through annRevs, if the months match up, add it into the month's total revenue
for product in annRevs.items():
    for sale in product[1]:
        key = sale[1]
        monthlyRevs[key] += sale[0]

months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']
mostMonth = 0
mostRev = 0
for i in range(12):
    if(monthlyRevs[i] > mostRev):
        mostRev = monthlyRevs[i]
        mostMonth = months[i]

print(f"{mostMonth} had the most revenue with $ {mostRev:,.2f}.")
