...
customerCount = 0
for i in customers:
    # if(isinstance(customers[i]["State"], int)):
    if(len(customers[i]["State"]) > 2 and len(customers[i]["Zip"]) == 2):
        switchCount += 1
    customerCount += 1

switchedPercentage = (switchCount/customerCount) * 100
switchedPercentage = "{:.2f}".format(switchedPercentage)
print(f"{switchedPercentage} % swapped \"State\" and \"Zip\" fields.")
