...
#Same as 08.py, just find the lowest instead of highest in the end
...
for i in productQuantities.items():
    for j in grossProductPrices.items():
        if(i[0] == j[0]):
            for k in i[1]:
                key = k[0]
                monthlyProfit[key] += k[1] * j[1]

#This line is really the only change from 07.py
#Just go through the months (0-11) and subtract the expense from the corresponding profit
for i in range(12):
    monthlyProfit[i] -= float(expenses[i]["Expenses"])

lowestProfit = monthlyProfit[0]
lowestMonth = 0
months = ['January', 'February', 'March', 'April', 'May', 'June', 'July', 'August', 'September', 'October', 'November', 'December']


for i in range(len(monthlyProfit)):
    if(monthlyProfit[i] < lowestProfit):
        lowestProfit = monthlyProfit[i]
        lowestMonth = months[i]


print(f"{lowestMonth} had the least net profit with $ {lowestProfit:,.2f}.")
